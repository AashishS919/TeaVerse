            ...
            if(paymentOption === "COD"){
                ...
                if(data.success){
                    ...
                    navigate('/my-orders')
                         ...
                         const { data } = await axios.post("http://localhost:4000/initiate-payment", {
                            amount: getCartAmount() + getCartAmount() * 2 / 100,
                            productId: generateUniqueId(), // Ensure this is available
                        });

                    if (data.url) {
                          window.location.href = data.url; // Redirect to eSewa
                    } else {
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={()=>{
                              navigate(`/shop/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)
                        ...
                        <button onClick={()=> removeFromCart(product._id)} className="cursor-pointer mx-auto">
                            <img src={assets.remove} alt="remove"
                            className="inline-block w-6 h-6"/>
                        </button>
                    </div>)
                )}

                <button onClick={()=> {navigate("/shop"); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 font-medium">
                    <img className="w-5 h-5 group-hover:-translate-x-1 transition" src={assets.leftarrow} alt="arrow"/>
                    Continue Shopping
                </button>
