...
const Orders = () => {
    ...
    const fetchOrders = async()=>{
        try {
            const {data} = await axios.get('/api/order/seller?paymentType=Online');
            if(data.success){
            ...
            {orders.map((order, index) => (
                <div key={index} className="flex flex-col md:items-center md:flex-row gap-5 justify-between p-5 max-w-4xl rounded-md border border-gray-300">
                    <div className="flex gap-5 max-w-80">
                        <img className="w-12 h-12 object-cover" src={assets.teabox_icon} alt="teabox" />
                        <div>
                    ...
                    <button
                     ...
                     className="text-red-600 text-sm border border-red-600 rounded px-3 py-1 hover:bg-red-600 hover:text-white transition"
                    >
                     Delete
                    </button>
